...
PDE: ∂u/∂t + v·∇u = κ∇²u
where v is velocity field, κ is diffusivity

Simplified 1D case:
∂u/∂t + v∂u/∂x = κ∂²u/∂x²

Initial condition: u(x,0) = exp(-50(x-0.5)²)
...
class AdvectionDiffusion1D:
    """1D Advection-Diffusion benchmark problem."""

    def __init__(self, v: float = 1.0, kappa: float = 0.01):
        """
        Initialize problem.
        ...
        self.v = v
        self.kappa = kappa
        self.name = "Advection-Diffusion 1D"
    ...
    def analytical_solution(self, x: np.ndarray, t: np.ndarray) -> np.ndarray:
        """
        Approximate analytical solution using traveling Gaussian.
        ...
        """
        # Traveling Gaussian with diffusion
        x_center = 0.5 + self.v * t
        width_sq = 1.0 / 50.0 + 2 * self.kappa * t
        return np.exp(-((x - x_center)**2) / width_sq)
    ...
    def pde_residual(self, network: torch.nn.Module, xt: torch.Tensor) -> torch.Tensor:
        """
        Compute PDE residual: ∂u/∂t + v∂u/∂x - κ∂²u/∂x²
        ...
        # PDE residual
        residual = u_t + self.v * u_x - self.kappa * u_xx
